...
contract RheaPublic is Ownable {
    ...
    uint256 public price; // price per rhea
    uint256 public saleAmount;
    uint256 public saleStartTime;
    uint256 public saleEndTime;
    uint256 public claimStartTime;
    uint256 public vestingTerm = 86400; // 1day
    ...
    modifier onlySaleTime() {
        require(
            saleStartTime <= block.timestamp && saleEndTime >= block.timestamp,
            "not sale time"
        );
        _;
    }
    ...
    modifier after3Month() {
        require(
            block.timestamp > saleEndTime &&
                block.timestamp - saleEndTime >= 90 days,
            "only after 3 months"
        );
        _;
    }

    modifier afterStakingSet() {
        require(stakingHelper != address(0), "no staking helper");
        _;
    }

    modifier afterSaleAmountSet() {
        require(saleAmount != 0, "no sale amount");
        _;
    }
    ...
    function setSaleAmount() external onlyOwner afterInitialized {
        require(saleAmount == 0, "cannot set sale amount twice");
        saleAmount = IERC20(rhea).balanceOf(address(this));
    }
    ...
    function claim() external afterInitialized afterClaimTime afterStakingSet {
        uint256 claimableRhea = getClaimableRhea(msg.sender);
    ...
    function refund() external afterInitialized afterSaleEnd {
    ...
    function getAssignedAmount(address _account) public view returns (uint256) {
        uint256 totalSalePrice = (price * saleAmount) / 1e9;

        if (totalDeposit > totalSalePrice) {
            uint256 depositAmount = deposit[_account];
            uint256 portion = FixedPoint
                .fraction(depositAmount, totalDeposit)
                .decode112with18()
                .div(1e9);
            uint256 assigned = (saleAmount * portion) / 1000000000;
            return assigned;
        } else {
            uint256 depositAmount = deposit[_account];
            uint256 assigned = FixedPoint
                .fraction(depositAmount, price)
                .decode112with18()
                .div(1e9);
            return assigned;
        }
    ...
    function getRefundAmount(address _account) public view returns (uint256) {
        uint256 assignedAmount = getAssignedAmount(_account);
        uint256 purchased = (price * assignedAmount) / 1e9;
        uint256 refundAmount = deposit[_account] -
            purchased -
            refunded[_account];
    ...
    function getClaimableRhea(address _address) public view returns (uint256) {
        uint256 assigned = getAssignedAmount(_address);

        if (block.timestamp <= claimStartTime) {
            return 0;
        }

        uint256 claimPhase = (block.timestamp - claimStartTime) / vestingTerm;
        claimPhase = claimPhase >= vestingPercent.length
            ? vestingPercent.length
            : claimPhase;

        uint256 claimableRatio = 0;

        for (uint256 i = 0; i < claimPhase; i++) {
            claimableRatio = claimableRatio + vestingPercent[i];
        }

        uint256 claimableRhea = ((assigned * claimableRatio) / 10000) -
            claimed[_address];

        return claimableRhea;
    }

    function sendToken(address _account) external onlyOwner afterSaleEnd {
    ...
    function sendTokenEmergency(address _account, uint256 _amount)
        external
        onlyOwner
        afterSaleEnd
    {
        require(
            ...
            "cannot transfer more than the deposit amount"
        );
        IERC20(token).transfer(_account, _amount);
    }

    function cleanUp(address _account) external onlyOwner after3Month {
        uint256 leftAmount = IERC20(rhea).balanceOf(address(this));
        IRHEA(rhea).burn(leftAmount);

        uint256 leftDAI = IERC20(token).balanceOf(address(this));
